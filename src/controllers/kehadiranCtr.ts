...
export const absen = async (req: Request, res: Response) => {
  try {
    ...
    const karyawan = await Karyawan.findById(req.user.id);
    ...
    if (jenis === 'pulang') {
      ...
      dataKehadiran = await Kehadiran.findOneAndUpdate({ karyawan: karyawan._id, datang: { $gte: jamMulai, $lt: jamAkhir }, pulang: { $exists: false } }, { $set: { pulang: Date.now() } }, { new: true })
...
export const semuaKehadiran = async (req: Request, res: Response) => {
  try {
    ...
    const dataKehadiran = await Kehadiran.find({}, { __v: 0 })
      ...
      .populate("karyawan", { password: 0, __v: 0, peran: 0 });
